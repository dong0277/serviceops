...
> **현재 상태:** GitHub 중심의 로컬 재현형 개인 포트폴리오 `v1.0.0` MVP를 완료했습니다. clean checkout, 백엔드 테스트 22개, 격리된 Playwright 흐름 12개, 영문·한글 스크린샷과 데모 GIF를 검증했습니다. 오너 예약 결과에는 서버 검색·필터·일정 정렬·페이지네이션을 구현했습니다. 공개 호스팅과 사람을 통한 스크린리더·실기기 검토는 이 릴리스의 검증 주장에 포함하지 않으며 향후 선택 작업으로 연기했습니다.
...
저장소는 단순한 모노레포 구조입니다. 하나의 Next.js 애플리케이션이 공개·고객·직원·오너 화면을 포함합니다. FastAPI는 인증, 권한, 테넌시, 예약 규칙의 기준이며 PostgreSQL은 Docker Compose로 로컬에서 실행됩니다. 포트폴리오는 이 공개 GitHub 저장소, 다국어 미디어, 재현 가능한 로컬 스택으로 제공하며 MVP에 실제 ServiceOps 호스팅이나 관리형 데이터베이스는 필요하지 않습니다.
...
- 공개 배포는 `v1.0.0` 범위에서 제외했습니다. 향후 호스팅 실험에는 새로운 명시적 결정과 배포 도메인 기준의 전체 보안 검증이 필요합니다.
...
1. 선택 사항: VoiceOver·추가 스크린리더 및 실기기 검토
2. 선택 사항: 쿠키·CORS·CSRF·도메인·속도 제한 검증을 포함한 무료 공개 배포 실험
3. 검증된 범용 프로젝트 패턴을 MVP 이후 별도 추출
